...
def scale_contour(contour, scale_factor):
    ...
    scaled_contour = scaled_contour.astype(int)  # Convert to integer type
...
def draw_line(img, contours):
    ...
    for contour in contours:
        ...
        cv2.line(image, (x, cy), (x + w, cy), (0, 255, 0), 1)  # Horizontal line
        cv2.line(image, (cx, y), (cx, y + h), (255, 0, 0), 1)  # Vertical line
...
def process_size(
    image,
    vertical_line_length_mm,
    vertical_line_length_px,
    total_height,
    total_width,
    contours,
):
    ...
    cv2.putText(
        image,
        f"Height {total_height * px_to_mm_vertical:.2f} mm",
        (x, y - 10),
    ...
    cv2.putText(
        image,
        f"Width {total_width * px_to_mm_vertical:.2f} mm",
        (x, y - 30),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.5,
        (0, 255, 0),
    ...
    if False:
        for contour in contours:
            ...
            for angle in range(0, 360, 60):
                ...
                # Draw line
                cv2.line(
                    image, (cx, cy), (end_x, end_y), (angle, angle, angle), 1
                )  # Lines in cyan
...
def convert_to_stl(contours, vertical_line_length_mm, vertical_line_length_px):
    ...
    pixel_to_mm = (
        vertical_line_length_mm / vertical_line_length_px
    )  # Example conversion factor (adjust as needed)
